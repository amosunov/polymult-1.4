...
\item Instead of accepting an upper \code{bound} on the size of the coefficients, the executable \code{polymult} is now accepting the size of that upper bound in bits (a.k.a. \code{bitsize}).
...
\item The subroutine \code{multiply(...)} in \code{mult.c} automatically creates three auxiliary folders where temporary files are being stored. Previously, these folders remained existing (though empty) after subroutine's termination. Now the last 6 lines of the subroutine ensure that they are deleted as well.
...
\begin{lstlisting}
./polymult [poly_degree] [number_of_files] [bundle] [bitsize] [resultname] [folder]
