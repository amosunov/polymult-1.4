...
void init_files(const ulong * primes, const uint total_primes,
		...
		const char * resultname, const uint series_total, const ulong * series_params, const char flags)
{
	...
	for (uint s = 0; s < files; s++)
	{
		...
		// If series_total > 0, initializing from a code
		if (series_total)
		{
			#pragma omp parallel
			{
				...
				if (series_total == 1)
				{
					memset(theta + n * (blocksize / max_threads), 0, sizeof(int) * (blocksize / max_threads + ((n == max_threads - 1) ? (blocksize % max_threads) : 0)));
					init_block_nabla(INIT_PARAMS, series_params[0], series_params[1], series_params[2], series_params[3], series_params[4]);
				}
				else if (series_total > 1)
				{
					memset(theta + n * (blocksize / max_threads), 0, sizeof(int) * (blocksize / max_threads + ((n == max_threads - 1) ? (blocksize % max_threads) : 0)));

					for (int j = 0; j < (series_total/2); j++)
					{
						init_block_nabla_product(INIT_PARAMS, series_params[10*j], series_params[10*j + 1], series_params[10*j + 2], series_params[10*j + 3], series_params[10*j + 4], series_params[10*j + 5], series_params[10*j + 6], series_params[10*j + 7], series_params[10*j + 8], series_params[10*j + 9]);
					}
				}
			}
		}
		// If series_total == 0, initializing from files
		else
		{
			sprintf(name, "temp%u", s);
		...
		const char * resultname, const char * folder, const uint series_total, const ulong * series_params)
{
	...
	{
		gettimeofday(&begin, NULL);
		printf("Initializing %lu*(q^%lu)*Nabla_{%lu, %lu}(q^%lu)...\n", series_params[0], series_params[1], series_params[2], series_params[3], series_params[4]);
		init_files(primes, total_primes, limit, files, bundle, bitsize, mod, name_g, 1, series_params, 0);
		gettimeofday(&end, NULL);
		exec_time = (end.tv_sec*1e6 + end.tv_usec) - (begin.tv_sec*1e6 + begin.tv_usec);
	...
	gettimeofday(&begin, NULL);
	printf("Initializing the second series...\n");
	init_files(primes, total_primes, limit, files, bundle, bitsize, mod, name_f, series_total - 1, series_params + 5, 0);
	gettimeofday(&end, NULL);
	exec_time = (end.tv_sec*1e6 + end.tv_usec) - (begin.tv_sec*1e6 + begin.tv_usec);
	printf("Initialized in %.3f sec.\n\n", exec_time / 1000000.0);
